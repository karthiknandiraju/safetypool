#!/usr/bin/env python3
"""Train canonical HighwayEnv baselines once per seed.
...
try:
    ...
    raise SystemExit(
        "HighwayEnv is not installed. Run: python -m pip install highway-env gymnasium"
    ) from exc
    ...
    try:
        for episode in range(args.train_episodes):
            ...
            for step in range(args.max_episode_steps):
                ...
                # MDP state. Bootstrap through it because HighwayEnv uses
...
def save_method(
    ...
    manifest = {
        ...
        "method": method,
        "method_label": METHOD_LABELS[method],
        "observation_source": "flattened_highway_kinematics_observation_only",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train canonical Epsilon, NoisyNet, and DQN+RND HighwayEnv baselines"
    )
