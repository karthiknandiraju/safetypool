#!/usr/bin/env python3
"""HighwayEnv Karthikeya27adv23 DQN with safety evidence.
...
This packaged policy is HighwayEnv-only and always uses the included
``HighwayEnvAdapter``.
...
def make_env(args, phase: str) -> HighwayEnvAdapter:
    """Create the package's fixed HighwayEnv environment."""
...
def verify_discrete_action_space(
    env, args, enforce_highway_config: bool = False
) -> int:
    ...
    if enforce_highway_config:
        expected_count = int(
            args.discrete_steering_dim * args.discrete_throttle_dim
        )
        if action_count != expected_count:
            raise RuntimeError(
                f"Configured HighwayEnv grid expects {expected_count} actions, "
...
def run_frozen_test_phase(
    ...
    try:
        test_action_count = verify_discrete_action_space(
            env, args, enforce_highway_config=env_factory is make_env
        )
    except Exception:
...
def run_experiment(
    ...
    try:
        action_count = verify_discrete_action_space(
            train_env,
            args,
            enforce_highway_config=env_factory is make_env,
        )
    except Exception:
        train_env.close()
        raise
...
def parse_args(argv: Optional[Sequence[str]] = None):
    parser = argparse.ArgumentParser(
        description=(
            "HighwayEnv Karthikeya27adv23 with retained blocked evidence and "
            "environment-sized safety masks"
        )
    )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    """Run the policy with the package's fixed HighwayEnv adapter."""
    args = parse_args(argv)
    env_factory = make_env
    safety_extractor = extract_safety_vector
    step_info_parser = parse_step_info
    args.environment_factory = "highway_env_adapter:HighwayEnvAdapter"
    args.safety_extractor = _callable_label(safety_extractor)
    args.step_info_parser = _callable_label(step_info_parser)
    args.environment_name = "HighwayEnv"
    args.observation_source = OBSERVATION_SOURCE
    args.uses_engine_object_safety_scan = True
    args.baseline_index_compatible = True
    ...
    print("=" * 76)
    print("HIGHWAYENV KARTHIKEYA27ADV23 RETAINED-EVIDENCE SAFETY POLICY")
    print("=" * 76)
